...
settings:SetWidth(220) settings:SetHeight(240) settings:SetPoint("CENTER", 0, 0)
settings:SetBackdrop({bgFile="Interface\\ChatFrame\\ChatFrameBackground", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border", tile=true, tileSize=16, edgeSize=16, insets={left=3,right=3,top=3,bottom=3}})
settings:SetBackdropColor(0,0,0,0.9) settings:SetMovable(true) settings:EnableMouse(true)
settings:RegisterForDrag("LeftButton") settings:SetScript("OnDragStart", function() this:StartMoving() end) settings:SetScript("OnDragStop", function() this:StopMovingOrSizing() end)
settings:Hide()
...
close:SetPoint("TOPRIGHT", 2, 2) close:SetScript("OnClick", function() settings:Hide() end)
...
importBtn:SetScript("OnClick", function()
    -- Look for the function globally, regardless of pfUI version
    local decoder = (pfUI and pfUI.api and pfUI.api.Base64Decode) or (Base64Decode)
    if decoder then
        local decoded = decoder(sd_profile_string)
        local func = loadstring(decoded)
        if func then func() ReloadUI() end
    else
        DEFAULT_CHAT_FRAME:AddMessage("|cff00ffffpfUI-SD:|r Manual override—decoder not found. Retrying...")
    end
end)

-- 4. SLASH COMMAND & LOGIC
SLASH_PFSD1 = "/pfsd"
SlashCmdList["PFSD"] = function() if settings:IsShown() then settings:Hide() else settings:Show() end end
...
-- Delayed Init to avoid the "API not found" error
local loader = CreateFrame("Frame")
loader:Hide()
loader:SetScript("OnUpdate", function()
    this.timer = (this.timer or 0) + arg1
    if this.timer > 5 then
        DEFAULT_CHAT_FRAME:AddMessage("|cff33ff99pfUI-SD:|r Profile ready. Use /pfsd.")
        this:Hide()
    end
end)
loader:Show()
