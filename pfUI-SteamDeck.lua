...
-- 3. CREATE SETTINGS WINDOW (Defined globally to fix 'nil' error)
PFSD_Settings = CreateFrame("Frame", "PFSD_SettingsFrame", UIParent)
PFSD_Settings:SetWidth(220) PFSD_Settings:SetHeight(240) PFSD_Settings:SetPoint("CENTER", 0, 0)
...
local importBtn = CreateFrame("Button", nil, PFSD_Settings, "UIPanelButtonTemplate")
...
importBtn:SetScript("OnClick", function()
    ...
    if decoder then
        local raw = decoder(sd_profile_string)
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cff00ffffpfUI-SD:|r Decoder not found. Ensure pfUI is enabled.")
    end
end)
